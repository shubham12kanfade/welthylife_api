  ...
  crudController
    ...
    .then((testData) => {
      ...
      req.body.testArray.forEach((element) => {
        obj.push({ profileId: testData._id, testId: element });
      });
      crudController
...
router.get("/master/by/profileId/:id", (req, res) => {
  log.debug("/api/");
  crudController
    .getOne(ProfileMaster, { _id: req.params.id })
    .then((resPkg) => {
      crudController
        .getbySortByPopulate(
          ProfileTests,
...
router.post("/add/tests/to/:profileId", (req, res) => {
  ...
  var obj = [];
  req.body.testArray.forEach((element) => {
    obj.push({ profileId: req.params.profileId, testId: element });
  });
  crudController
    .insertMultiple(ProfileTests, obj)
    .then((resData) => {
...
router.delete("/master/:id", auth, (req, res) => {
  ...
  crudController
    ...
    .then((userData) => {
      crudController
        .deleteMulti(ProfileTests, { profileId: req.params.id })
        .then((resData) => {
  ...
  LabProfiles.aggregate([
    ...
    { $unwind: "$tests" },
    {
      $group: {
        _id: "$_id",
        mrp: { $first: "$mrp" },
        discountOffer: { $first: "$discountOffer" },
        PPL: { $first: "$PPL" },
        icon: { $first: "$profile.icon" },
        title: { $first: "$profile.title" },
        description: { $first: "$profile.description" },
        precaution: { $first: "$profile.precaution" },
        lab: {
          $first: {
            _id: "$labId",
            icon: "$lab.icon",
